...
class EnvProtector {
  ...
  setupProtection() {
    ...
    // SECURITY FIX: Intercept Reflect API methods that can bypass the proxy
    this.interceptReflectAPI(envProxy);
    
    // SECURITY FIX: Intercept Object methods that can bypass the proxy
    this.interceptObjectMethods(envProxy);
  }
  ...
  interceptReflectAPI(envProxy) {
    ...
    try {
      ...
      // Intercept Reflect.get
      Reflect.get = function(target, prop, receiver) {
        if (target === process.env || target === envProxy) {
          if (typeof prop === 'string' && self.isProtected(prop)) {
            const packageName = self.getCallingPackage();
            const check = self.checkAccess(prop, packageName);
            
            if (!check.allowed) {
              self.logAccess(prop, packageName, 'REFLECT_GET', check);
              if (!self.config.mode?.alertOnly) {
                return '[PROTECTED]';
              }
            }
      ...
      // Intercept Reflect.getOwnPropertyDescriptor
      Reflect.getOwnPropertyDescriptor = function(target, prop) {
        if (target === process.env || target === envProxy) {
          if (typeof prop === 'string' && self.isProtected(prop)) {
            ...
            if (!check.allowed) {
              self.logAccess(prop, packageName, 'REFLECT_DESCRIPTOR', check);
              if (!self.config.mode?.alertOnly) {
                const desc = originalReflectGetOwnPropertyDescriptor(target, prop);
        ...
        return originalReflectGetOwnPropertyDescriptor(target, prop);
      };
    } catch (e) {
      if (!this.silent) {
        console.warn('[Env Protector] Could not intercept Reflect API:', e.message);
      }
    }
  }
  
  interceptObjectMethods(envProxy) {
    const self = this;
    ...
    try {
      ...
      // Intercept Object.getOwnPropertyNames
      Object.getOwnPropertyNames = function(obj) {
        const names = originalObjectGetOwnPropertyNames(obj);
        
        if (obj === process.env || obj === envProxy) {
          return names.filter(name => {
            if (self.isProtected(name)) {
              const packageName = self.getCallingPackage();
              const check = self.checkAccess(name, packageName);
              return check.allowed;
            }
            return true;
          });
      ...
      Object.getOwnPropertyDescriptor = function(obj, prop) {
        if (obj === process.env || obj === envProxy) {
          if (typeof prop === 'string' && self.isProtected(prop)) {
            ...
            if (!check.allowed) {
              self.logAccess(prop, packageName, 'OBJECT_DESCRIPTOR', check);
              if (!self.config.mode?.alertOnly) {
                const desc = originalObjectGetOwnPropertyDescriptor(obj, prop);
                if (!desc) return undefined;
                return {
                  value: undefined,
                  writable: desc.writable,
        ...
        return originalObjectGetOwnPropertyDescriptor(obj, prop);
      };
      
      // Intercept Object.entries
      Object.entries = function(obj) {
        if (obj === process.env || obj === envProxy) {
          const entries = originalObjectEntries(obj);
      ...
      // Intercept Object.values
      Object.values = function(obj) {
        if (obj === process.env || obj === envProxy) {
          // For values, we need to filter based on keys
          const entries = originalObjectEntries(obj);
      ...
      Object.assign = function(target, ...sources) {
        const filteredSources = sources.map(source => {
          if (source === process.env || source === envProxy) {
            const filtered = {};
        ...
        return originalObjectAssign(target, ...filteredSources);
      };
    } catch (e) {
      if (!this.silent) {
        console.warn('[Env Protector] Could not intercept Object methods:', e.message);
      }
    }
  }
}
