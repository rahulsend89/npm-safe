...
const { getInstance: getWorkerThreadsInterceptor } = require('./worker-threads-interceptor');
const fs = require('fs');
const path = require('path');

// Read version from package.json (cross-platform compatible)
let CURRENT_VERSION = '2.0.5'; // Fallback version
try {
...
class FirewallCore {
      ...
      if (originalFs.existsSync(pkgPath)) {
        ...
        if (installedVersion !== currentVersion) {
          ...
          if (isOlder) {
            ...
            console.log('');
            console.log(`  Version ${currentVersion} includes improvements and fixes.`);
            console.log('');
  ...
  isPackageManager() {
    try {
      ...
      // Check if running inside npm, yarn, or pnpm as a package manager operation
      // SECURITY FIX: Only consider it a package manager context for actual package operations
      // (install, ci, add, etc.), NOT for run-script which executes user code
      const isNpmPackageOp = npmCommand === 'install' || 
                              npmCommand === 'ci' || 
                              npmCommand === 'add' ||
                              npmCommand === 'update' ||
      ...
      const isNpm = mainModule.includes('/npm/') || 
                    ...
                    title === 'npm' ||
                    (npmExecPath.includes('npm') && isNpmPackageOp) ||
                    isNpmPackageOp;
  ...
  initialize() {
    ...
    if (this.config.network?.enabled) {
      ...
      if (!this.silent) {
        ...
        // NOTE: GitHub API Monitor logs its own status when initialized
        // No need to duplicate the logs here
      }
    }
    ...
    // Initialize worker_threads interceptor (prevent worker bypass)
    const workerInterceptor = getWorkerThreadsInterceptor();
    workerInterceptor.setupInterception();
  ...
  setupCleanup() {
    ...
    // Both init.mjs and firewall-core.js may try to register handlers
    const CLEANUP_REGISTERED = Symbol.for('node.firewall.cleanup.registered');
    
    if (global[CLEANUP_REGISTERED]) {
      // Cleanup already registered (likely by init.mjs), skip
      return;
    }
  ...
  checkFileAccess(operation, filePath, packageName = null) {
    ...
    const debugFs = process.env.NODE_FIREWALL_DEBUG_FS === '1';
    ...
    if (operation === 'READ') {
      ...
      const normalizedResolvedPath = this.normalizePathForMatch(resolvedPath);
      const blocked = this.config.filesystem?.blockedReadPaths || [];

      if (debugFs) {
        console.error(`[Firewall][FS DEBUG] READ filePath=${filePath} resolvedPath=${resolvedPath}`);
        console.error(`[Firewall][FS DEBUG] READ normalizedFilePath=${normalizedFilePath} normalizedResolvedPath=${normalizedResolvedPath}`);
      }
      ...
      for (const pattern of blocked) {
        ...
        const normalizedPatternNoTrailingSlash = normalizedPatternForMatch.endsWith('/')
          ? normalizedPatternForMatch.slice(0, -1)
          : normalizedPatternForMatch;
        
        // Check both original path and resolved path to catch relative and absolute paths
        // Also check startsWith for exact directory matches (pattern may have trailing /)
        const matchesPattern = normalizedFilePath.includes(normalizedPatternForMatch) ||
                               normalizedResolvedPath.includes(normalizedPatternForMatch) ||
                               normalizedFilePath === normalizedPatternNoTrailingSlash ||
                               normalizedResolvedPath === normalizedPatternNoTrailingSlash ||
                               normalizedFilePath.startsWith(normalizedPatternNoTrailingSlash + '/') ||
                               normalizedResolvedPath.startsWith(normalizedPatternNoTrailingSlash + '/');

        if (debugFs) {
          console.error(`[Firewall][FS DEBUG] READ blockedReadPaths pattern=${pattern} normalizedPattern=${normalizedPatternForMatch} matches=${matchesPattern}`);
        }
    ...
    if (operation === 'WRITE' || operation === 'CREATE') {
      ...
      // SECURITY FIX: Check allowedPaths BEFORE blockedWritePaths
      // This allows explicitly allowed paths to override blocked patterns
      const allowedPaths = this.config.filesystem?.allowedPaths || [];
      if (allowedPaths.length > 0) {
        const isInAllowedPath = allowedPaths.some(allowedPattern => {
          const normalizedPattern = this.normalizePathForMatch(allowedPattern);
          // Check if file path starts with or includes the allowed pattern
          return normalizedFilePath.startsWith(normalizedPattern) ||
                 ...
                 normalizedResolvedPath.includes(normalizedPattern);
        });
        
        if (isInAllowedPath) {
          result = { allowed: true, reason: 'allowed_path' };
          this.logAudit('FILESYSTEM', operation, filePath, result, packageName);
          return result;
        }
      }
      ...
      if (debugFs) {
        console.error(`[Firewall][FS DEBUG] WRITE/CREATE filePath=${filePath} resolvedPath=${resolvedPath}`);
        console.error(`[Firewall][FS DEBUG] WRITE/CREATE normalizedFilePath=${normalizedFilePath} normalizedResolvedPath=${normalizedResolvedPath}`);
      }
      ...
      for (const pattern of blocked) {
        ...
        const normalizedPatternForMatch = this.normalizePathForMatch(normalizedPattern);
        const normalizedFilePathNoTrailingSlash = normalizedFilePath.replace(/\/+$/, '');
        const normalizedResolvedPathNoTrailingSlash = normalizedResolvedPath.replace(/\/+$/, '');
        ...
        const matchesPattern = normalizedFilePathNoTrailingSlash.startsWith(normalizedPatternForMatch + '/') ||
                              normalizedFilePathNoTrailingSlash === normalizedPatternForMatch ||
                              normalizedResolvedPathNoTrailingSlash.startsWith(normalizedPatternForMatch + '/') ||
                              normalizedResolvedPathNoTrailingSlash === normalizedPatternForMatch ||
                              normalizedFilePath.includes(normalizedPatternForMatch) ||
                              normalizedResolvedPath.includes(normalizedPatternForMatch);

        if (debugFs) {
          console.error(`[Firewall][FS DEBUG] WRITE blockedWritePaths pattern=${pattern} normalizedPattern=${normalizedPatternForMatch} matches=${matchesPattern}`);
        }
      ...
      const filePathForExtMatch = process.platform === 'win32' ? String(filePath).toLowerCase() : String(filePath);
      for (const ext of blockedExt) {
        const extForMatch = process.platform === 'win32' ? String(ext).toLowerCase() : String(ext);
        if (filePathForExtMatch.endsWith(extForMatch)) {
          return { 
            allowed: false, 
