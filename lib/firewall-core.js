...
class FirewallCore {
  ...
  checkFileAccess(operation, filePath, packageName = null) {
    ...
    if (this.config.mode?.strictMode) {
      ...
      const path = require('path');
      const resolvedPath = path.resolve(filePath);
      console.log('[STRICT DEBUG] filePath:', filePath);
      console.log('[STRICT DEBUG] resolvedPath:', resolvedPath);
      console.log('[STRICT DEBUG] allowedPaths:', allowed);
      const isAllowed = allowed.some(pattern => {
        const matches = filePath.includes(pattern) || resolvedPath.includes(pattern);
        console.log('[STRICT DEBUG] pattern:', pattern, 'matches:', matches);
        return matches;
      });
      console.log('[STRICT DEBUG] isAllowed:', isAllowed);
