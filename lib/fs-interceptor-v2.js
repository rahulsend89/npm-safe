...
const FIREWALL_MODULE_NAMES = ['audit-logger.js', 'behavior-monitor.js', 'network-monitor.js', 'github-api-monitor.js', 'firewall-core.js', 'child-process-interceptor.js', 'fs-interceptor-v2.js', 'firewall-hardening-fortress.js'];
...
function isFirewallInternalStack(stack) {
  ...
  for (const line of stackLines) {
    ...
    if (filePath.includes('<anonymous>') || filePath.includes('node:') || filePath.includes('internal/') || filePath.includes('(native)')) {
      continue;
    }
    ...
    if (absolutePath.startsWith(FIREWALL_LIB_DIR + path.sep) || absolutePath === FIREWALL_LIB_DIR) {
...
const originalFs = {...fs};
['readFileSync', 'writeFileSync', 'appendFileSync', 'existsSync', 'statSync', 'lstatSync', 'readdirSync', 'createWriteStream', 'createReadStream', 'unlinkSync', 'mkdirSync', 'rmdirSync', 'rmSync', 'renameSync', 'copyFileSync', 'openSync', 'opendirSync', 'readSync', 'writeSync', 'readFile', 'writeFile', 'appendFile', 'unlink', 'mkdir', 'rmdir', 'rm', 'rename', 'copyFile', 'open', 'opendir', 'read', 'write', // SECURITY FIX: Include symlink/link and realpath methods
  'symlinkSync', 'linkSync', 'symlink', 'link', 'realpathSync', 'realpath', // SECURITY FIX: Include access methods for existence detection prevention
  'accessSync', 'access'].forEach(method => {
...
class FileSystemInterceptor {
    ...
    try {
      ...
      if (ppid) {
        ...
        if (process.platform === 'win32') {
          ...
          try {
            parentCmd = execSync(`wmic process where processid=${ppid} get name /format:value`, {
              encoding: 'utf8',
              windowsHide: true,
              timeout: 1000,
  ...
  setupInterception() {
    ...
    if (fs.symlink) fs.symlink = this.wrapSymlink('symlink');
    if (fs.linkSync) fs.linkSync = this.wrapSync('linkSync', 'WRITE');
    if (fs.link) fs.link = this.wrapAsync('link', 'WRITE');
    ...
    if (fs.promises) {
      ...
      const operationMap = {
        ...
        readdir: 'READ',
        open: 'READ',
        opendir: 'READ', // SECURITY FIX: Additional methods
        realpath: 'READ',
        readlink: 'READ',
        symlink: 'WRITE',
        link: 'WRITE'
      ...
      const promisesProxy = new Proxy(originalPromises, {
        get(target, prop) {
          ...
          return async function(...args) {
            ...
            // SECURITY FIX: Special handling for two-path operations (symlink, link, rename, copyFile)
            // These operations have both a source/target and a destination that must be validated
            const twoPathOps = ['symlink', 'link', 'rename', 'copyFile'];
            let destPath = null;
            let resolvedDestPath = null;
            let destShouldIntercept = false;

            if (twoPathOps.includes(prop) && args[1]) {
              destPath = self.extractPath(args[1]);
              ...
              destShouldIntercept = self.shouldIntercept(destPath, prop) || self.shouldIntercept(resolvedDestPath, prop);
            }

            if (!self.shouldIntercept(filePath, prop) && !self.shouldIntercept(realFilePath, prop) && !destShouldIntercept) {
            ...
            // SECURITY FIX: For two-path operations, also validate the destination path
            if (destPath) {
              const destCheck = self.checkAccess('WRITE', destPath);
              const destResolvedCheck = (destPath !== resolvedDestPath) ? self.checkAccess('WRITE', resolvedDestPath) : destCheck;
  ...
  wrapSync(method, operation) {
    ...
    return function(...args) {
      ...
      // SECURITY FIX: For two-path methods (copyFileSync, renameSync, etc.),
      // also check if the DESTINATION should be intercepted
      const twoPathMethodsForIntercept = ['copyFileSync', 'renameSync', 'copyFile', 'rename', 'link', 'linkSync', 'symlink', 'symlinkSync'];
      let destShouldIntercept = false;
      if (twoPathMethodsForIntercept.includes(method) && args[1]) {
        ...
        destShouldIntercept = self.shouldIntercept(destPath, method) || self.shouldIntercept(resolvedDestPath, method);
      }

      // Check if EITHER source path OR destination path should be intercepted
      if (!self.shouldIntercept(filePath, method) && !self.shouldIntercept(resolvedPath, method) && !destShouldIntercept) {
  ...
  wrapStream(method, operation) {
    ...
    return function (filePath, options) {
      ...
      // SECURITY FIX: Resolve realpath to handle symlinks
      let realPath = resolvedPath;
      try {
        if (originalFs.existsSync(resolvedPath)) {
          realPath = originalFs.realpathSync(resolvedPath);
        }
      } catch (e) {
      }
      if (!self.shouldIntercept(resolvedPath, method) && !self.shouldIntercept(realPath, method)) {
        return original.call(fs, filePath, options);
      }

      const check = self.checkAccess(operation, resolvedPath);
      const realCheck = (resolvedPath !== realPath) ? self.checkAccess(operation, realPath) : check;
      const effectiveCheck = (!check.allowed) ? check : ((!realCheck.allowed) ? realCheck : check);
  ...
  /**
   * SECURITY FIX: Wrap symlink operations to prevent bypass via symlinks
   * Symlinks can be used to bypass path restrictions by creating a link
   * in an allowed location that points to a blocked path
   *
   * Handles both synchronous (symlinkSync) and asynchronous (symlink) calls
   * For async: fs.symlink(target, path, [type], callback)
   * For sync: fs.symlinkSync(target, path, [type])
   */
  wrapSymlink(method) {
    const self = this;
    ...
    return function (target, linkPath, ...rest) {
      // Extract callback for async operations
      // Callback is the last argument, or second-to-last if type is specified
      let callback = null;
      let typeArg = null;

      if (!isSync && rest.length > 0) {
        const lastArg = rest[rest.length - 1];
        if (typeof lastArg === 'function') {
          callback = lastArg;
          // If there's a type argument, it's before the callback
          if (rest.length > 1) {
            typeArg = rest[0];
          }
        } else {
          // No callback, just type argument
          typeArg = lastArg;
        }
      } else if (isSync && rest.length > 0) {
        // Sync with type argument
        typeArg = rest[0];
      }
      ...
      if (!effectiveTargetCheck.allowed) {
        ...
        const error = self.createAccessError(`Firewall: Cannot create symlink to blocked path - ${resolvedTarget}`);
      ...
      if (!linkCheck.allowed) {
        ...
        const error = self.createAccessError(`Firewall: ${linkCheck.reason} - ${resolvedLink}`);
  ...
  shouldIntercept(filePath, method) {
    ...
    const normalizedFilePath = this.normalizePathForMatch(filePath);
    ...
    const isFirewallOutputFile = filePath.includes('fs-firewall') || filePath.includes('firewall-report') || filePath.includes('firewall-audit') || filePath.includes('firewall-zero-trust');
    ...
    if (isFirewallOutputFile) {
      ...
      // Skip the first 3 frames which are part of the interception mechanism:
      // 1. Error creation (this line)
      // 2. shouldIntercept method
      // 3. wrapSync/wrapAsync method
      // We want to check if the ACTUAL CALLER (frame 4+) is firewall internal
      const stackLines = stack.split('\n');
      const callerStack = stackLines.slice(3).join('\n');
      const isFirewallInternal = isFirewallInternalStack(callerStack);
    ...
    const isInstallMode = process.env.FIREWALL_INSTALL_MODE === '1' || process.env.npm_command === 'install' || process.env.npm_command === 'ci' || process.env.npm_lifecycle_event === 'install';
    ...
    if (normalizedFilePath.includes('/.npm/') || normalizedFilePath.includes('/.yarn/') || normalizedFilePath.includes('/.pnpm/') || normalizedFilePath.includes('/node_modules/') || normalizedFilePath.includes('/.cache/') || normalizedFilePath.includes('/.nvm/') || normalizedFilePath.includes('/var/folders/')) {  // macOS temp (used by npm)
      return false;
    }
    ...
    if (isInstallMode) {
      ...
      if (normalizedFilePath.endsWith('package-lock.json') || normalizedFilePath.endsWith('yarn.lock') || normalizedFilePath.endsWith('pnpm-lock.yaml') || normalizedFilePath.endsWith('package.json') || normalizedFilePath.endsWith('.npmrc') ||  // npm needs to read .npmrc for auth
        normalizedFilePath.endsWith('.gitconfig')) {  // npm needs .gitconfig for git dependencies
        return false;
      }
    }
    ...
    if (this.firewall?.config?.filesystem) {
      ...
      for (const pattern of [...blockedReadPaths, ...blockedWritePaths]) {
        ...
        const normalizedPatternForMatch = this.normalizePathForMatch(pattern);
        const normalizedPatternNoTrailingSlash = normalizedPatternForMatch.endsWith('/') ? normalizedPatternForMatch.slice(0, -1) : normalizedPatternForMatch;
        if (normalizedFilePath.includes(normalizedPatternForMatch) || normalizedFilePath === normalizedPatternNoTrailingSlash || normalizedFilePath.startsWith(normalizedPatternNoTrailingSlash + '/')) {
          return true; // DO intercept - this path is explicitly blocked
        }
      }

      // SECURITY FIX: Check blocked extensions BEFORE auto-allowing project directory
      // This ensures .sh, .py, etc. files are blocked even in the project directory
      const blockedExtensions = this.firewall.config.filesystem.blockedExtensions || [];
      for (const ext of blockedExtensions) {
        const extForMatch = process.platform === 'win32' ? String(ext).toLowerCase() : String(ext);
        if (normalizedFilePath.endsWith(extForMatch)) {
          return true; // DO intercept - this extension is blocked
        }
      }
    }

    // Auto-allow project directory (only if not sensitive AND not in blockedPaths AND not blocked extension)
    if (this.projectDir) {
      const normalizedProjectDir = this.normalizePathForMatch(this.projectDir);
      if (normalizedProjectDir && normalizedFilePath.startsWith(normalizedProjectDir)) {
        return false;
      }
    }
  ...
  isSensitivePattern(filePath, isInstallMode = false) {
    ...
    const criticalSensitive = ['/.ssh/',      // SSH keys
      '/.aws/',      // AWS credentials
      '/.gnupg/',    // GPG keys
    ...
    if (criticalSensitive.some(pattern => normalizedFilePath.includes(pattern))) {
      return true;
    }

    // CONDITIONAL: These are blocked except when npm legitimately needs them
    if (!isInstallMode) {
      const conditionalSensitive = ['/.env',           // Environment files (app should use dotenv)
        '/.gitconfig',     // Git config
        '/.github/workflows/', // CI workflows
      ];

      if (conditionalSensitive.some(pattern => normalizedFilePath.includes(pattern))) {
        return true;
      }
    }
  ...
  isExecutableFile(filePath, content = null) {
    ...
    if (content) {
      const contentStr = Buffer.isBuffer(content) ? content.toString('utf8', 0, 100) : String(content).substring(0, 100);
  ...
  checkAccess(operation, filePath, content = null) {
    if (!this.firewall) {
      // SECURITY: Fail closed - if firewall failed to initialize, deny all access by default
      // This prevents an insecure state where filesystem access is unrestricted
      return {
        allowed: false,
        reason: 'firewall_not_ready',
        severity: 'critical',
        message: 'Firewall failed to initialize; blocking by default'
      };
    }
    ...
    if (isFirewallOutputFile && (operation === 'WRITE' || operation === 'CREATE' || operation === 'DELETE')) {
      ...
      // Skip wrapper frames to check actual caller
      const stackLines = stack.split('\n');
      const callerStack = stackLines.slice(3).join('\n'); // Skip Error + checkAccess + wrapSync
      const isFirewallInternal = isFirewallInternalStack(callerStack);
    ...
    if (operation === 'WRITE' || operation === 'CREATE') {
      ...
      for (const ext of blockedExt) {
        if (filePath.endsWith(ext)) {
          return {
            allowed: false, reason: 'blocked_extension', extension: ext, severity: 'high'
          };
        }
      }
    }
