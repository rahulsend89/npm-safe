...
class FileSystemInterceptor {
  ...
  shouldIntercept(filePath, method) {
    ...
    if (isFirewallOutputFile) {
      ...
      const isFirewallInternal = stack.includes('audit-logger.js') ||
                                  stack.includes('behavior-monitor.js') ||
                                  stack.includes('network-monitor.js') ||
                                  stack.includes('github-api-monitor.js') ||
                                  stack.includes('firewall-core.js');
  ...
  checkAccess(operation, filePath, content = null) {
    ...
    if (isFirewallOutputFile && (operation === 'WRITE' || operation === 'CREATE' || operation === 'DELETE')) {
      ...
      const isFirewallInternal = stack.includes('audit-logger.js') ||
                                  ...
                                  stack.includes('github-api-monitor.js') ||
                                  stack.includes('firewall-core.js') ||
                                  stack.includes('child-process-interceptor.js');
