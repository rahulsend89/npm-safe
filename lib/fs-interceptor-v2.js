...
class FileSystemInterceptor {
  ...
  shouldIntercept(filePath, method) {
    ...
    if (normalizedFilePath.includes('/.npm/') || 
        normalizedFilePath.includes('/.yarn/') || 
        normalizedFilePath.includes('/.pnpm/') ||
        normalizedFilePath.includes('/node_modules/') ||
        normalizedFilePath.includes('/.cache/') ||
        normalizedFilePath.includes('/.nvm/') ||
        normalizedFilePath.includes('/var/folders/')) {  // macOS temp (used by npm)
      return false;
    }
    ...
    if (isInstallMode) {
      ...
      if (normalizedFilePath.endsWith('package-lock.json') ||
          normalizedFilePath.endsWith('yarn.lock') ||
          normalizedFilePath.endsWith('pnpm-lock.yaml') ||
          normalizedFilePath.endsWith('package.json') ||
          normalizedFilePath.endsWith('.npmrc') ||  // npm needs to read .npmrc for auth
    ...
    if (this.firewall?.config?.filesystem) {
      ...
      for (const pattern of [...blockedReadPaths, ...blockedWritePaths]) {
        ...
        if (normalizedFilePath.includes(normalizedPatternForMatch) || 
