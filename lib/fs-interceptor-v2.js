...
class FileSystemInterceptor {
  constructor() {
    ...
    // Initialize firewall immediately (needed for interception to work)
    // Use setImmediate if we need async, but try sync first
    try {
      ...
      // If circular dependency, initialize on next tick
      setImmediate(() => {
        try {
          const { getInstance } = require('./firewall-core');
          this.firewall = getInstance();
