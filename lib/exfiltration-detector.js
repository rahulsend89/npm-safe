...
class ExfiltrationDetector {
  ...
  trackSensitiveFileRead(filePath) {
    ...
    // SECURITY FIX: Limit Map size to prevent memory leaks
    const MAX_SENSITIVE_READS = 1000;
    ...
    // SECURITY FIX: If Map is still too large, remove oldest entries
    if (this.recentSensitiveReads.size > MAX_SENSITIVE_READS) {
      // Sort by timestamp and remove oldest
      const entries = Array.from(this.recentSensitiveReads.entries())
        .sort((a, b) => a[1] - b[1]); // Sort by timestamp
      
      // Remove oldest entries until under limit
      const toRemove = entries.slice(0, entries.length - MAX_SENSITIVE_READS);
