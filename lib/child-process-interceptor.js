...
class ChildProcessFirewall {
  ...
  wrappedSpawn(command, args, options) {
    ...
    // Always create a plain object copy of the environment
    if (opts.env) {
      try {
        const plainEnv = {};
        for (const key of Object.keys(opts.env)) {
          plainEnv[key] = opts.env[key];
        }
        opts.env = plainEnv;
      } catch (e) {
      ...
      if (isDependencyLifecycle) {
        ...
        if (allThreats.length > 0) {
          ...
          if (criticalThreats.length > 0) {
            ...
            const error = new Error(`Firewall blocked dangerous command in ${npmPackageName} ${lifecycleEvent}`);
            error.code = 'EACCES';
            throw error;
          }
    ...
    if (allThreats.length === 0) {
      ...
      if (!opts.env) {
        // Create plain object copy of process.env
        const plainEnv = {};
        for (const key of Object.keys(process.env)) {
          plainEnv[key] = process.env[key];
        }
