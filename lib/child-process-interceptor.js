...
class ChildProcessFirewall {
  ...
  checkCommand(operation, command, caller) {
    ...
    const readOperators = ['cat ', 'less ', 'more ', 'head ', 'tail ', 'grep ', 'awk ', 'sed ', 'cut ', 'strings '];
    ...
    const scriptExtensions = ['.sh', '.bash', '.zsh', '.py', '.rb', '.pl', '.php'];
    const isScriptExecution = scriptExtensions.some(ext => command.includes(ext));
    ...
    if (isScriptExecution && blockedReadPaths.length > 0) {
      ...
      try {
        ...
        const scriptMatch = command.match(/([^\s]+\.(sh|bash|zsh|py|rb|pl|php))/);
        if (scriptMatch) {
          ...
          if (fs.existsSync(absoluteScriptPath)) {
            ...
            for (const blockedPath of blockedReadPaths) {
              ...
              if (scriptContent.includes(pathPattern)) {
