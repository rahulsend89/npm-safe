...
class ChildProcessFirewall {
  ...
  getFirewallLoaderArgs() {
    ...
    const loaderPath = supportsImport
      ...
      : path.join(__dirname, 'legacy-loader.mjs');

    const loaderUrl = process.platform === 'win32' || !supportsImport
    ...
    if (!supportsImport) {
      args.push('-r', path.join(__dirname, 'fs-interceptor-v2.js'));
      args.push('-r', path.join(__dirname, 'child-process-interceptor.js'));
    }
  ...
  hasFirewallArgs(argsArray) {
    if (!Array.isArray(argsArray)) return false;
    return argsArray.some(a => {
      const s = String(a || '');
      return s.includes('init.mjs') || s.includes('legacy-loader.mjs') || s.includes('fs-interceptor-v2.js');
    });
  }

  shouldInjectIntoNode(stack, argsArray) {
    const stackStr = String(stack || '');
    const isNpmOperation = stackStr.includes('@npmcli/arborist') ||
                          stackStr.includes('@npmcli/run-script') ||
                          stackStr.includes('@npmcli/promise-spawn') ||
                          stackStr.includes('node-gyp');

    const isNativeBuild = stackStr.includes('node-gyp') ||
                          (Array.isArray(argsArray) && argsArray.some(a => String(a).includes('node-gyp') || String(a).includes('gyp')));

    return !isNpmOperation && !isNativeBuild;
  }
  ...
  wrappedSpawn(command, args, options) {
    ...
    if (allThreats.length === 0) {
      ...
      if (this.isNodeCommand(command) && !this.hasFirewallArgs(argsArray) && this.shouldInjectIntoNode(stack, argsArray)) {
        const injectedArgs = this.getFirewallLoaderArgs();
        argsArray = [...injectedArgs, ...argsArray];
        args = argsArray;
        if (!opts.env) opts.env = this.copyEnv(process.env);
        opts.env.NODE_FIREWALL = '1';
      }
  ...
  wrappedExecFile(file, args, options, callback) {
    ...
    const stack = new Error().stack;
    let argsArray = args;
    const optsForInject = options && typeof options === 'object' ? options : undefined;

    if (this.isNodeCommand(file) && Array.isArray(argsArray) && !this.hasFirewallArgs(argsArray) && this.shouldInjectIntoNode(stack, argsArray)) {
      const injectedArgs = this.getFirewallLoaderArgs();
      argsArray = [...injectedArgs, ...argsArray];
      args = argsArray;

      if (optsForInject) {
        if (!optsForInject.env) {
          optsForInject.env = this.copyEnv(process.env);
        } else {
          optsForInject.env = this.copyEnv(optsForInject.env);
        }
        optsForInject.env.NODE_FIREWALL = '1';
      }
    }
  ...
  wrappedExecFileSync(file, args, options) {
    ...
    return this.originals.execFileSync.call(this, file, args, opts);
  }

  checkCommand(operation, command, caller) {
    const normalizedCommand = this.normalizePathForMatch(String(command || ''));

    // SECURITY: Check blockedWritePaths from config
    // This prevents bypassing fs protection via shell commands (e.g., echo HACKED > /etc/passwd)
    const blockedWritePaths = this.config?.filesystem?.blockedWritePaths || [];
    const writeOperators = ['>', '>>', 'tee ', 'dd ', 'cp ', 'mv ', 'rm ', 'unlink', 'del ', 'erase ', 'truncate'];
    
    for (const blockedPath of blockedWritePaths) {
      // Normalize path for matching (remove leading slash for relative matching)
      const pathPattern = blockedPath.replace(/^\//, '');

      const normalizedBlockedPath = this.normalizePathForMatch(blockedPath);
      const normalizedBlockedPathNoTrailingSlash = normalizedBlockedPath.replace(/\/+$/, '');

      if (command.includes(pathPattern) ||
          (normalizedBlockedPathNoTrailingSlash && normalizedCommand.includes(normalizedBlockedPathNoTrailingSlash))) {
        // Check if command performs write operations
        const hasWriteOp = writeOperators.some(op => command.includes(op));
        
        if (hasWriteOp) {
          this.logAccess('DENIED', operation, command, caller, [{ 
            name: 'Blocked Write Path',
            description: `Attempt to write to blocked path: ${blockedPath}`
          }]);
          console.error(`\n  BLOCKED: Attempt to write to blocked path: ${blockedPath}`);
          return false;
        }
      }
    }
    
    // SECURITY: Check blockedReadPaths from config
    // This prevents reading sensitive files via shell commands (e.g., cat ~/.ssh/id_rsa)
    const blockedReadPaths = this.config?.filesystem?.blockedReadPaths || [];
    const readOperators = ['cat ', 'less ', 'more ', 'head ', 'tail ', 'grep ', 'awk ', 'sed ', 'cut ', 'strings ', 'type '];
    ...
    for (const blockedPath of blockedReadPaths) {
      ...
      if (command.includes(pathPattern) ||
          (normalizedBlockedPathNoTrailingSlash && normalizedCommand.includes(normalizedBlockedPathNoTrailingSlash))) {
        // Check if command performs read operations
        const hasReadOp = readOperators.some(op => command.includes(op));
        
        if (hasReadOp) {
          this.logAccess('DENIED', operation, command, caller, [{ 
            name: 'Blocked Read Path',
            description: `Attempt to read from blocked path: ${blockedPath}`
          }]);
          console.error(`\n  BLOCKED: Attempt to read from blocked path: ${blockedPath}`);
          return false;
        }
      }
    }
    
    // SECURITY FIX: Check if executing a script that reads from blocked paths
    // Scripts (.sh, .bash, .py, etc.) can bypass command-level blocking by hiding
    // dangerous commands inside the script file
    const scriptExtensions = ['.sh', '.bash', '.zsh', '.py', '.rb', '.pl', '.php', '.bat', '.cmd'];
    const isScriptExecution = scriptExtensions.some(ext => normalizedCommand.includes(ext));
    
    if (isScriptExecution && blockedReadPaths.length > 0) {
      // Try to read the script content and check for blocked path access
      try {
        // Use original fs to avoid recursion with intercepted fs
        const path = require('path');
        
        // Extract script path from command (first file-like argument)
        const scriptMatch = command.match(/\"([^\"]+\.(sh|bash|zsh|py|rb|pl|php|bat|cmd))\"|([^\s]+\.(sh|bash|zsh|py|rb|pl|php|bat|cmd))/i);
        if (scriptMatch) {
          const scriptPath = scriptMatch[1] || scriptMatch[3];
          const absoluteScriptPath = path.isAbsolute(scriptPath) 
            ? scriptPath 
            : path.resolve(process.cwd(), scriptPath);
          
          if (originalFs.existsSync(absoluteScriptPath)) {
            const scriptContent = originalFs.readFileSync(absoluteScriptPath, 'utf8');
            const normalizedScriptContent = this.normalizePathForMatch(scriptContent);
            
            // Check if script content contains blocked paths
            for (const blockedPath of blockedReadPaths) {
              const normalizedBlockedPath = this.normalizePathForMatch(blockedPath);
              const normalizedBlockedPathNoTrailingSlash = normalizedBlockedPath.replace(/\/+$/, '');
              
              if (normalizedBlockedPathNoTrailingSlash && normalizedScriptContent.includes(normalizedBlockedPathNoTrailingSlash)) {
                // Script contains reference to blocked path
                this.logAccess('DENIED', operation, command, caller, [{ 
                  name: 'Script Contains Blocked Path',
                  description: `Script ${scriptPath} contains reference to blocked path: ${blockedPath}`
                }]);
                console.error(`\n  BLOCKED: Script execution contains blocked path: ${blockedPath}`);
                return false;
              }
            }
            
            // Also check for blocked write paths in script
            const blockedWritePaths = this.config?.filesystem?.blockedWritePaths || [];
            for (const blockedPath of blockedWritePaths) {
              const normalizedBlockedPath = this.normalizePathForMatch(blockedPath);
              const normalizedBlockedPathNoTrailingSlash = normalizedBlockedPath.replace(/\/+$/, '');
              ...
              if (normalizedBlockedPathNoTrailingSlash && normalizedScriptContent.includes(normalizedBlockedPathNoTrailingSlash)) {
                this.logAccess('DENIED', operation, command, caller, [{ 
                  name: 'Script Contains Blocked Write Path',
                  description: `Script ${scriptPath} contains reference to blocked write path: ${blockedPath}`
                }]);
                console.error(`\n  BLOCKED: Script execution contains blocked write path: ${blockedPath}`);
                return false;
              }
            }
          }
        ...
        // If we can't read the script, log warning but don't block
        // (the script might not exist yet or be dynamically generated)
      }
    }
    
    // Check for dangerous patterns
    const threats = this.dangerousPatterns.filter(p => p.pattern.test(command) || (process.platform === 'win32' && p.pattern.test(normalizedCommand)));
