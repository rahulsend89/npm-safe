...
class ChildProcessFirewall {
  ...
  checkCommand(operation, command, caller) {
    ...
    // SECURITY FIX: For synchronous operations (exec, execSync), we can't use async prompts
    // Return false (deny) in interactive mode for synchronous operations
    ...
    if (operation === 'exec' || operation === 'execSync') {
      ...
      this.logAccess('DENIED', operation, command, caller, threats);
      console.error('\n  BLOCKED shell command (interactive mode not supported for exec):', command);
      return false;
    }
