...
class BehaviorMonitor {
  ...
  constructor(config, silent = false, fsOverride = null) {
    ...
    // INSTALL MODE DETECTION: Relax thresholds during npm install
  ...
  trackFileRead(filePath) {
    ...
    // Skip counting reads from safe directories (no security concern)
    if (this.isSafeDirectory(filePath)) {
      return { allowed: true, reason: 'safe_directory' };
    }
    
    // Only count reads that might be security-relevant
    global[METRICS_SYMBOL].fileReads++;
    
    // Only check for SUSPICIOUS reads (sensitive files), not just any read
    if (this.isSuspiciousRead(filePath)) {
      this.recordSuspicious('SUSPICIOUS_FILE_READ', filePath);
    }
    
    // Check limits (with filtering)
  ...
  trackFileWrite(filePath) {
    ...
    // Skip counting writes to safe directories (no security concern)
    if (this.isSafeDirectory(filePath)) {
      return { allowed: true, reason: 'safe_directory' };
    }
    
    // Only count writes that might be security-relevant
    global[METRICS_SYMBOL].fileWrites++;
    
    // Only check for SUSPICIOUS writes, not just any write
  ...
  isSafeDirectory(filePath) {
    // These directories are safe - no need to count or alert
    const safePatterns = [
      '/node_modules/',
      '/.npm/',
      '/.yarn/',
    ...
    return safePatterns.some(p => filePath.includes(p));
  }
  
  isSuspiciousRead(filePath) {
    // Only flag reads of actually sensitive files
    ...
    // .env files are only suspicious if not in project directory
    ...
    return sensitivePatterns.some(p => filePath.includes(p));
  }
  
  trackNetworkRequest(url, method = 'GET') {
    global[METRICS_SYMBOL].networkRequests++;
    
    // Only record if actually suspicious
  ...
  checkThreshold(metric, context) {
    // Filter out already alerted metrics to prevent log spam
    if (!this.alertedMetrics) this.alertedMetrics = new Set();
    if (this.alertedMetrics.has(metric)) return;

    const thresholds = this.config.behavioral?.alertThresholds || {};
    let threshold = thresholds[metric];
    
    // INSTALL MODE: Use much higher thresholds during npm install
    if (this.isInstallMode && threshold) {
      const installMultiplier = {
        fileReads: 100,    // 100x more reads during install
        fileWrites: 100,   // 100x more writes during install
        networkRequests: 10, // 10x more network during install
        processSpawns: 20   // 20x more spawns during install (postinstall scripts)
      };
      threshold = threshold * (installMultiplier[metric] || 10);
    }
    ...
    if (threshold && this.metrics[metric] >= threshold) {
      ...
      if (!this.silent && shouldAlert) {
        console.warn(`\n[BEHAVIOR ALERT] Unusual activity detected (Threshold Exceeded)`);
        console.warn(`   Package: ${this.currentPackage || 'unknown'}`);
        console.warn(`   Metric: ${metric} = ${this.metrics[metric]} (threshold: ${threshold})`);
        console.warn(`   Note: This alert is shown once per metric to prevent flooding.`);
      }
  ...
  checkHardLimit(metric, operationType, context) {
    ...
    let limit = maxLimits[metric];
    
    // INSTALL MODE: Use much higher limits during npm install
    if (this.isInstallMode && limit) {
      const installMultiplier = {
        networkRequests: 10,
        fileWrites: 100,   // npm install writes thousands of files
        processSpawns: 20
      };
      limit = limit * (installMultiplier[metric] || 10);
    }
    
    if (limit && this.metrics[metric] > limit) {
      // Only log hard limit errors once per metric to avoid flooding
      if (!this.alertedMetrics) this.alertedMetrics = new Set();
      const limitKey = `limit_${metric}`;
      
      if (!this.alertedMetrics.has(limitKey)) {
        const shouldAlert = this.config.reporting?.alertOnSuspicious !== false;
        ...
        if (!this.silent && shouldAlert) {
          ...
          console.error(`   Note: Future violations of this limit will be blocked silently.`);
        }
        this.alertedMetrics.add(limitKey);
        
        // Only record the first violation to avoid filling the log with duplicates
  ...
  recordSuspicious(type, details) {
    ...
    // SECURITY FIX: Limit array sizes to prevent memory leaks
    const MAX_SUSPICIOUS_OPS = 1000;
    const MAX_BEHAVIOR_LOG = 5000;
    
    this.suspiciousOperations.push(entry);
    if (this.suspiciousOperations.length > MAX_SUSPICIOUS_OPS) {
      // Remove oldest entries (FIFO)
      this.suspiciousOperations.shift();
    }
    
    this.behaviorLog.push(entry);
    if (this.behaviorLog.length > MAX_BEHAVIOR_LOG) {
      // Remove oldest entries (FIFO)
