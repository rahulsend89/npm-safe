...
class ConfigLoader {
  ...
  getDefaults() {
    return {
      ...
      commands: {
        blockedPatterns: [],  // User-defined patterns from config take priority
        allowedCommands: ['npm', 'node', 'git', 'make', 'cmake', 'gcc', 'g++', 'python', 'python3']
      },
      environment: {
        protectedVariables: [],  // User must explicitly define protected vars
